...
The address bar updates automatically after yard edits, renames, and configuration changes. The app compresses a full yard snapshot with gzip and puts URL-safe Base64 into `?yard=1.…`; copy the browser URL to share the current yard. Updates replace the current history entry, so editing does not fill the Back button's history. Reloading a URL generated in this tab resumes the browser draft normally. Opening a copied link previews the yard; **Import as new configuration** adds it without replacing existing configurations. Incoming links stay unchanged until imported or dismissed. Older additive maps require an explicit update/import; incompatible maps and malformed links leave saved yards intact. A copied link remains a fixed snapshot. It includes the yard's location and notes, is readable by anyone with the URL, and may appear in browser history and server logs. Use the app's hosted address when sharing with someone on another device.

Share URLs have an 8,000-character portability budget, including the app address. This is not a guarantee that every messaging service or server accepts them. Sample full files compressed to about 1,800 URL characters for a blank yard, 3,500 for 25 plants, and 7,300 for 100 plants; 250 plants required about 14,600 characters. Notes, care history, coverage, and schedules change these sizes. If a yard exceeds the limit, the app removes the `yard` parameter and warns that you need to download it to share it. Browser autosave and **Save / Download** continue working. If later edits make it fit again, the parameter returns and the warning clears. Decoding also limits expanded data to 2 MiB. Sharing uses the browser's built-in Compression Streams API; no storage service or compression dependency is involved.
...
New yards start with the owner's documented RD-900-R program schedule. Program A runs Monday/Wednesday/Friday at 08:00, with Starts 2 and 3 off and station runtimes 1–9 of 10, 35, 35, 15, 20, 35, 35, 15, and 10 minutes. Program B retains 50 minutes on Stations 2 and 3; Program C retains 50 on Station 6 and 55 on Station 7. All other B/C runtimes are off. B and C have all weekdays selected but no start times, so neither starts automatically. Station 9's watering area was not found during manual testing and appears unused; its programmed runtime is retained without inventing coverage.

For an existing yard, **Load documented schedule** in **Rain Dial schedule** replaces only the program days, starts, and runtimes, preserving entered budgets and execution settings. It clears the previous schedule's verification dates and marks the replacement proposed until checked on the controller. **Undo** restores the previous settings in one step. Existing browser configurations and imported files retain their schedules until you choose this action. Timezone, budgets, delay settings, and physical coverage mappings still need to be entered or confirmed; the 210-minute Program A sum is programmed runtime, not measured watering.
