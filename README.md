...
**Create share link** compresses a full yard snapshot with gzip and puts URL-safe Base64 into `?yard=1.…`. Copy the resulting URL to share it. Opening a link previews the yard; **Import as new configuration** adds it without replacing existing configurations. Older additive maps require an explicit update/import; incompatible maps and malformed links leave saved yards intact. A link is a fixed snapshot, so regenerate it after edits. It includes the yard's location and notes, is readable by anyone with the URL, and may appear in browser history and server logs. Use the app's hosted address when sharing with someone on another device.

Share URLs have an 8,000-character portability budget, including the app address. This is not a guarantee that every messaging service or server accepts them. Sample full files compressed to about 1,800 URL characters for a blank yard, 3,500 for 25 plants, and 7,300 for 100 plants; 250 plants required about 14,600 characters. Notes, care history, coverage, and schedules change these sizes. The app reports the actual length and directs oversized yards to **Save / Download**. Decoding also limits expanded data to 2 MiB. Sharing uses the browser's built-in Compression Streams API; no storage service or compression dependency is involved.
